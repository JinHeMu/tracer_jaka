...
// 【新增】定义导纳控制器的前缀
const std::string ADMITTANCE_PREFIX = "jaka_admittance_controller/"; 
const bool BOOL_CHAINED = false;
...
JointTrajectoryController::command_interface_configuration() const
{
  ...
  for (const auto & joint_name : command_joint_names_)
  {
    for (const auto & interface_type : params_.command_interfaces)
    {
      if (BOOL_CHAINED)
      {
        conf.names.push_back(ADMITTANCE_PREFIX + joint_name + "/" + interface_type);
      } 
      else
      {
  ...
  const rclcpp_lifecycle::State &)
{
  ...
  if(BOOL_CHAINED)
  {
    ...
    for (const auto & interface : params_.command_interfaces)
    {
      ...
      // 【修改开始】 手动查找带有前缀的接口，替代 get_ordered_interfaces
      auto & current_interface_list = joint_command_interface_[index];
      current_interface_list.clear();
      current_interface_list.reserve(command_joint_names_.size());

      for (const auto & joint_name : command_joint_names_)
      {
        // 构造完整的接口名：jaka_admittance_controller/joint_1/position
        std::string full_name = ADMITTANCE_PREFIX + joint_name + "/" + interface;
        
        // 在系统分配给我们的接口列表 (command_interfaces_) 中查找这个名字
        auto interface_it = std::find_if(
          command_interfaces_.begin(), command_interfaces_.end(),
          [&full_name](const auto & cmd_iface) {
            return cmd_iface.get_name() == full_name;
          });

        if (interface_it != command_interfaces_.end())
        {
          current_interface_list.emplace_back(*interface_it);
        }
        else
        {
          RCLCPP_ERROR(
            logger, "Expected command interface '%s' not found.", full_name.c_str());
          return CallbackReturn::ERROR;
        }
      }
      // 【修改结束】
