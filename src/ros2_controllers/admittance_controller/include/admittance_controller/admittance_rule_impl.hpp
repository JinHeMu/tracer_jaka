...
void AdmittanceRule::apply_parameters_update()
{
  ...
  vec_to_eigen(parameters_.admittance.target_wrench, admittance_state_.target_wrench_control);
  vec_to_eigen(parameters_.admittance.force_control_axes, admittance_state_.force_control_axes);
...
bool AdmittanceRule::calculate_admittance_rule(AdmittanceState & admittance_state, double dt)
{
  ...
  // 将 F_base, X_dot, X 变换到控制坐标系
  Eigen::Matrix<double, 6, 1> F_ctrl, X_dot_ctrl, X_ctrl;
  F_ctrl.block<3,1>(0,0) = rot_base_control.transpose() * F_base.block<3,1>(0,0);
  F_ctrl.block<3,1>(3,0) = rot_base_control.transpose() * F_base.block<3,1>(3,0);
  X_dot_ctrl.block<3,1>(0,0) = rot_base_control.transpose() * X_dot.block<3,1>(0,0);
  X_dot_ctrl.block<3,1>(3,0) = rot_base_control.transpose() * X_dot.block<3,1>(3,0);
  X_ctrl.block<3,1>(0,0) = rot_base_control.transpose() * X.block<3,1>(0,0);
  X_ctrl.block<3,1>(3,0) = rot_base_control.transpose() * X.block<3,1>(3,0);

  // 在控制坐标系下，分轴计算加速度
  // D_diag 和 K_diag 是控制坐标系下的对角元素（未旋转前的原始值）
  Eigen::Matrix<double, 6, 1> D_diag = admittance_state.damping;
  Eigen::Matrix<double, 6, 1> K_diag = admittance_state.stiffness;

  Eigen::Matrix<double, 6, 1> X_ddot_ctrl;
  for (int i = 0; i < 6; ++i)
  {
    if (admittance_state.force_control_axes[i] > 0.5)
    {
      // 恒力控制：无刚度项，跟踪目标力
      X_ddot_ctrl[i] = admittance_state.mass_inv[i] *
        (admittance_state.target_wrench_control[i] + F_ctrl[i] -  D_diag[i] * X_dot_ctrl[i]);
    }
    else
    {
      // 导纳柔顺：保留刚度项
      X_ddot_ctrl[i] = admittance_state.mass_inv[i] *
        (F_ctrl[i] - D_diag[i] * X_dot_ctrl[i] - K_diag[i] * X_ctrl[i]);
    }
  }

  // 变换回基坐标系
  Eigen::Matrix<double, 6, 1> X_ddot;
  X_ddot.block<3,1>(0,0) = rot_base_control * X_ddot_ctrl.block<3,1>(0,0);
  X_ddot.block<3,1>(3,0) = rot_base_control * X_ddot_ctrl.block<3,1>(3,0);
