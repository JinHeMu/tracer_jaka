...
struct AdmittanceState
{
  explicit AdmittanceState(size_t num_joints)
  {
    ...
    force_control_axes.setZero();
    target_wrench_control.setZero();
  ...
  Eigen::Matrix<double, 6, 1> target_wrench_control; // 【新增】目标力(在控制坐标系下)
  Eigen::Matrix<double, 6, 1> force_control_axes;    // 力控轴标记
