...
bool jointStates(JointValue joint_pose)
{
    ...
    for (int i = 0; i < 6; i++)
    {
        bool ret = joint_pose.jVal[i] * 180 / PI - 0.2 < robotstatus.joint_position[i] * 180 / PI && robotstatus.joint_position[i] * 180 / PI < joint_pose.jVal[i] * 180 / PI + 0.2;
...
int main(int argc, char *argv[])
{
    ...
    ros::Publisher joint_states_pub = nh.advertise<sensor_msgs::JointState>("/arm/joint_states", 10);
